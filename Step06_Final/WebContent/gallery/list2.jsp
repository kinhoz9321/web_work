	...
	$(window).on("scroll",function(){
		...
		if(isBottom){
			...
			*2페이지까지만 나오는 이유 (중복 로딩을 방지하기 위해 짠 로직의 버그)
			...
			//$.ajax(); 요청에 대한 응답은 {}함수로 응답이된다. 더보기 버튼을 만들거라면 수정하기
			$.ajax({
...
서버로 부터 데이터를 받아와서 html로 해석을 해서 append div에 추가한다.
특정 데이터를 서버로 부터 받아와서 페이지 전환없이 추가로 넣어주는 일이 많다.
ajax 필요.

예시> gmail - 페이지 이동 없이 페이지 전환 (ajax로 받아왔을 것) 화면을 부분업데이트 하고 있다.
https://mail.google.com/mail/u/0/?tab=rm&ogbl#inbox 받은 편지함
https://mail.google.com/mail/u/0/?tab=rm&ogbl#starred 별표 편지함
#=책갈피
페이지 이동은 없고, 책갈피만 바뀌었다.
ajax 확실히 공부하기 대부분의 웹브라우저는 ajax를 사용한다.

더보기 버튼을 눌러서 페이지 로딩이 되게 할 수도 있다.
더보기 버튼이 더 쉽다고 함. 도전해보기!
--%>