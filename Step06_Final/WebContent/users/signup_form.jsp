...
<script>
	/*상황에 따라 정규표현식을 바꾸면 된다. 검증은 사실 프론트 앤드 영역 하지만 백엔드가 해야할수도 있다.*/
	...
	/*
	필요한걸 제대로 입력했는지 수시로 확인해줘야 한다.
	여부를 관리해줘야 한다.
	아이디, 비밀번호, 이메일 전부.
	그걸 관리하려고 변수를 설정함.
	처음은 false 조건부로 조작해서 반응 보이기.
	이걸 잘 할 수 있다면 프로그래밍 비스므리하게 한 것...
	*/
	...
	//이메일 확인란
	//id가 email인 요소에 input(입력) 이벤트가 일어 났을 때 실행할 함수 등록 (콘솔창에 input 출력)
	...
	$("#id").on("input", function(){
		...
		//만일 문자열이 영소문자로 시작하지 않고 최소5,최대10글자 미만이 아니라면
